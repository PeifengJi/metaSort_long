...
# QUAST: quality assessment tool for genome assemblies, Bioinformatics (2013) 29 (8): 1072-1075.
...
def get_logger(name=False):
    
    if name:
        _loggers[name] = QLogger(name)
        return _loggers[name]
    else:
        existName = _loggers.keys()[0]
        return _loggers[existName]
...
class QLogger(object):
    ...
    def error(self, message='', exit_with_code=0, to_stderr=False, indent='', fake_if_nested_run=False):
        ...
        if message:
            ...
            if exit_with_code:
                msg += "\nYou can check the log file to find the error info."
                #self.info(sys._getframe().f_code.co_name)
                #self.info(sys._getframe().f_back.f_code.co_name)
        else:
            msg = ''
    ...
    def print_version(self, to_stderr=False):
        version, build = config.softVersion()
        if to_stderr:
    ...
    def print_system_info(self):
        ...
        try:
            import platform
            self._logger.info("  OS: " + platform.platform() + " (%s)" % config.platform_name)
